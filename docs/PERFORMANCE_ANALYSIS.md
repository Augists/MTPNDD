...
1. **最佳配置**：1 worker 性能最好（25.422s），比无 worker 快 10.2%
   - LACE 框架初始化带来优化（可能是内存分配、缓存预热等）
   - 单 worker 避免了多线程调度开销

2. **并行扩展性差**：2-4 workers 性能接近，说明：
   - 当前串行实现限制了并行收益
   - 操作之间依赖性强，难以并行化

3. **过度并行损失**：
   - 6 workers：性能开始下降（27.711s，仅快 1.0%）
   - 8 workers：严重下降（36.163s，慢 22.6%）
   - 原因：LACE 调度开销 > 并行收益

4. **与 RECORDING 关系**：
   - RECORDING=ON (0 workers): 28.143s
   - RECORDING=OFF (0 workers): 28.002s
   - 统计开销仅约 0.5%，可以接受

### 推荐配置
- **生产环境**：使用 1-2 workers 以获得最佳性能
- **开发/调试**：使用 0 workers + RECORDING=ON 以获取详细统计

### 优化方向
- 当前串行实现是瓶颈，需要重新设计并行策略
- 可能的并行点：
  - 不同 field 的节点创建（粗粒度并行）
  - BDD 操作的批量执行
  - 操作缓存的分区（减少锁竞争）
